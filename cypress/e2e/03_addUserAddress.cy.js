...
describe("E2E-Add new address",{ testIsolation: false }, () => {
    ...
    it('Should successfully go to address bok tab', () => {
        ...
        cy.getElementWithClassBase().should('exist').and('contain', 'Add New Address');
    });
   ...
   it('Should check if the fields contain the entered values', () => {
       address.firstname.should('have.value', Cypress.env('expectedValues').firstname);
       address.lastname.should('have.value', Cypress.env('expectedValues').lastname);
       address.companyField.should('have.value', Cypress.env('expectedValues').company);
       address.phoneField.should('have.value', Cypress.env('expectedValues').phoneNumber);
       address.streetAddressField.should('have.value', Cypress.env('expectedValues').street);
       address.cityField.should('have.value', Cypress.env('expectedValues').city);
       address.zipCodeField.should('have.value', Cypress.env('expectedValues').zipCode);
       address.countryField.should('have.value', Cypress.env('expectedValues').country);
       address.stateField.should('have.value', Cypress.env('expectedValues').state);
    }) 
    it('Should click on save address button and confirm that address has been added', () => {
       address.saveAddressButton.click();
       cy.getElementByDiv("message-succes").should('exist').and('contain', 'You saved the address.');
